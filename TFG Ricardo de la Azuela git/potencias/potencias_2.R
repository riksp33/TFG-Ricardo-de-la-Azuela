...
results = list(title = "Resultados de potencia del test para distintos tamaños muestrales. Escenario 2")
...
for (n in n_values) {
  ...
  for (i in 1:MC) {
    ...
    casos = rnorm(n, 0.44,1.4)
    auc_base = parametric_auc(controles, casos)
    youden_base = parametric_youden(controles, casos)
    loc_base = parametric_loc(controles, casos)
    param_base = EtaBinormal(controles, casos)
    kernel_opt_base = EtaKernel(controles, casos, "optimo", mesh_size = 300)
    kernel_hscv_base = EtaKernel(controles, casos, "hscv", mesh_size = 300)
    ...
    for (b in 1:B) {
      ...
      auc_bootstrap[b] = parametric_auc(controles_b, casos_b)
      youden_bootstrap[b] = parametric_youden(controles_b, casos_b)
      loc_bootstrap[b] = parametric_loc(controles_b, casos_b)
      binormal_bootstrap[b] = EtaBinormal(controles_b, casos_b)
      kernel_opt_bootstrap[b] = EtaKernel(controles_b, casos_b, "optimo", mesh_size = 300)
      kernel_hscv_bootstrap[b] = EtaKernel(controles_b, casos_b, "hscv", mesh_size = 300)
    }
...
full_path = file.path(dir_path,'potencias_2.json' )
