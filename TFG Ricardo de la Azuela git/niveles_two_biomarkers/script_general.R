...
scenario_n_two_biomarkers = function(m_1_enfermos, m_2_enfermos, v_1_enfermos, v_2_enfermos, boxcox, num)
{
  ...
  covariance_enfermos_values = c(0.0)
  
  results = list(title = paste("Resultados de nivel del test con múltiples marcadores para distintos tamaños muestrales. Escenario", num))
  ...
  for (covariance_enfermos in covariance_enfermos_values) {
    ...
    for (n in n_values) {
      ...
      #mismas correlaciones, las muestras tienen que ser igual distribuidas desde el principio
      cov_matrix_sanos = matrix(c(v_1_sanos, covariance_enfermos, 
                                  covariance_enfermos, v_2_sanos), nrow = 2)
  ...
  filename = paste0('tabla_', num,'.json')
  full_path = file.path(dir_path, filename )
  write(json, file = full_path)
  
}

scenario_n_two_biomarkers(0.36, 0.36, 1, 1, FALSE, 1)
scenario_n_two_biomarkers(1.19, 1.19, 1, 1, FALSE, 2)
scenario_n_two_biomarkers(1.81, 1.81, 1, 1, FALSE, 3)
